...
function CmdTestOutPut:init(filename)
	if filename ~= nil then
		f = io.open(filename, "w+b")
...
function CmdTestOutPut.EndCase(self, bRun, bSuccess, iTime, strLabelName)
	...
	if bSuccess then
		...
		TestResultValue.tc["flag"] = true
	else
		table.insert(self.stat.tFailedName, strLabelName)
		self:Message("%s %s (%d ms)", self:getFMTStr("failed"), strLabelName, iTime)
		TestResultValue.tc["flag"] = false
	end
	
	self:outerList("EndCase", bRun, bSuccess, iTime, strLabelName)	
	
	TestResultValue.tc["tc_name"] = strLabelName
	table.insert(TestResultValue.testcase, TestResultValue.tc)
	TestResultValue.tc["failedtxt"] = TestResultValue.failedtxt
...
function CmdTestOutPut.EndSuite(self, iNumber, strSuiteName, iTime, iFailedNum)
	...
	TestResultValue.ts["ts_name"] = strSuiteName
	TestResultValue.ts["total_num"] = iNumber
	TestResultValue.ts["failed_num"] = iFailedNum
	TestResultValue.ts["testcases"] = TestResultValue.testcase

	table.insert(TestResultValue.testsuite, TestResultValue.ts)
...
function CmdTestOutPut.FailedTxt(self, ...)
	...
	table.insert(TestResultValue.failedtxt, ...)
end

function CmdTestOutPut.Message(self, ...)
...
function CmdTestOutPut.StaticInfo(self)
	...
	TestResultValue.testresult["total_case"] = self.stat.iTotalCase
	TestResultValue.testresult["total_suite"] = self.stat.iTotalSuite
	TestResultValue.testresult["total_pass"] = self.stat.iTotalPassed
	TestResultValue.testresult["testsuites"] = TestResultValue.testsuite
	local str = encode (TestResultValue.testresult, { indent = true })
    if self.json.f then 
		self.json.f:write(str) 
		self.json.f:flush()
	end	
